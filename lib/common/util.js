 ...
 * @param  {object}    resObj       The vm which just changed
 * @param  {function}  cb           callback function which takes err param
 */
function publishChange(publisher, resource, subResources, uuid, resObj, cb) {
    assert.object(publisher, 'publisher');
    assert.string(resource, 'resource');
    assert.arrayOfString(subResources, 'subResources');
    assert.uuid(uuid, 'uuid');
    if (typeof (resObj) === 'function') {
        cb = resObj;
    } else {
        assert.object(resObj, 'resourceObj');
    }
    assert.func(cb, 'cb');
    ...
    if (resObj) {
        changeItem.resourceObject = resObj;
    }
